...
const TOPIC_QUESTIONS = {
  DSA: [
    { question: "What is an Array?", answer: "An array is a linear data structure that stores elements in contiguous memory locations. It allows O(1) random access but O(n) insertion/deletion.", difficulty: "Easy" },
    { question: "Explain Stack and Queue.", answer: "Stack follows LIFO (Last In First Out) while Queue follows FIFO (First In First Out). Stack uses push/pop, Queue uses enqueue/dequeue.", difficulty: "Easy" },
    { question: "What is a Linked List? How does it differ from an Array?", answer: "A Linked List stores elements in nodes with pointers to the next node. Unlike arrays, it allows O(1) insertion/deletion but O(n) access. No contiguous memory required.", difficulty: "Easy" },
    { question: "What is Binary Search? What is its time complexity?", answer: "Binary Search divides a sorted array in half repeatedly to find a target. Time complexity is O(log n). It requires the array to be sorted.", difficulty: "Easy" },
    { question: "Explain the concept of recursion with an example.", answer: "Recursion is when a function calls itself with a smaller subproblem. Example: factorial(n) = n * factorial(n-1). Every recursive function needs a base case to stop.", difficulty: "Medium" },
    { question: "What is a Binary Tree vs a Binary Search Tree?", answer: "A Binary Tree is a tree where each node has at most 2 children. A BST additionally satisfies: left child < parent < right child, enabling O(log n) search.", difficulty: "Medium" },
    { question: "What is Dynamic Programming? Give an example.", answer: "DP solves problems by breaking them into overlapping subproblems and storing results (memoization/tabulation). Example: Fibonacci, Longest Common Subsequence, Knapsack.", difficulty: "Medium" },
    { question: "Explain Bubble Sort and its time complexity.", answer: "Bubble Sort repeatedly swaps adjacent elements if they are in wrong order. Time complexity is O(n²) worst/average case, O(n) best case. Space complexity O(1).", difficulty: "Easy" },
    { question: "What is a Hash Table? How does collision handling work?", answer: "A Hash Table maps keys to values using a hash function for O(1) average lookup. Collisions are handled via chaining (linked lists at each bucket) or open addressing (probing).", difficulty: "Medium" },
    { question: "What is the difference between BFS and DFS?", answer: "BFS (Breadth First Search) explores level by level using a Queue, good for shortest path. DFS (Depth First Search) explores depth-first using a Stack/recursion, good for cycle detection.", difficulty: "Medium" },
    { question: "What is a Heap? Explain Min-Heap and Max-Heap.", answer: "A Heap is a complete binary tree satisfying the heap property. Min-Heap: parent ≤ children (root is minimum). Max-Heap: parent ≥ children (root is maximum). Used in priority queues.", difficulty: "Medium" },
    { question: "What is the time complexity of QuickSort? When does it degrade?", answer: "QuickSort average case is O(n log n), worst case O(n²) when the pivot is always the smallest/largest element (already sorted array). Space is O(log n) average.", difficulty: "Hard" },
    { question: "Explain the concept of Graph and its representations.", answer: "A Graph consists of vertices and edges. Represented as Adjacency Matrix (O(V²) space) or Adjacency List (O(V+E) space). Can be directed/undirected, weighted/unweighted.", difficulty: "Medium" },
    { question: "What is Merge Sort? How does it differ from Quick Sort?", answer: "Merge Sort divides array in half, recursively sorts, then merges — always O(n log n). Unlike QuickSort it's stable and guarantees O(n log n) but needs O(n) extra space.", difficulty: "Medium" },
    { question: "What is the difference between a Stack and a Heap in memory?", answer: "Stack memory stores function calls and local variables (LIFO, automatically managed). Heap memory is for dynamic allocation (malloc/new), manually managed, slower but larger.", difficulty: "Hard" },
  ],
  DBMS: [
    { question: "What is Normalization?", answer: "Normalization organizes data to reduce redundancy and improve integrity. Forms: 1NF (atomic values), 2NF (no partial dependency), 3NF (no transitive dependency), BCNF.", difficulty: "Medium" },
    { question: "What is a Primary Key?", answer: "A Primary Key uniquely identifies each record in a table. It must be unique and not null. A table can have only one primary key.", difficulty: "Easy" },
    { question: "What is the difference between DELETE, TRUNCATE, and DROP?", answer: "DELETE removes specific rows (can rollback, triggers fire). TRUNCATE removes all rows fast (can rollback in some DBs, no triggers). DROP removes the entire table structure.", difficulty: "Medium" },
    { question: "What are ACID properties in DBMS?", answer: "Atomicity (all or nothing), Consistency (data integrity maintained), Isolation (transactions don't interfere), Durability (committed data persists). These ensure reliable transactions.", difficulty: "Medium" },
    { question: "What is a Foreign Key?", answer: "A Foreign Key is a column that references the Primary Key of another table, establishing a relationship and enforcing referential integrity between tables.", difficulty: "Easy" },
    { question: "What is the difference between INNER JOIN and OUTER JOIN?", answer: "INNER JOIN returns rows with matching values in both tables. OUTER JOIN (LEFT/RIGHT/FULL) returns matched rows plus unmatched rows from one or both tables with NULLs.", difficulty: "Medium" },
    { question: "What is an Index in a database? When should you use it?", answer: "An Index speeds up data retrieval by creating a data structure on a column. Use for frequently queried columns, but avoid over-indexing as it slows INSERT/UPDATE/DELETE.", difficulty: "Medium" },
    { question: "What is a Transaction? What are isolation levels?", answer: "A Transaction is a unit of work that is atomic. Isolation levels: Read Uncommitted, Read Committed, Repeatable Read, Serializable — balance between consistency and performance.", difficulty: "Hard" },
    { question: "What is the difference between SQL and NoSQL?", answer: "SQL databases are relational with fixed schemas (MySQL, PostgreSQL). NoSQL databases are non-relational with flexible schemas (MongoDB, Redis). NoSQL scales horizontally better.", difficulty: "Medium" },
    { question: "What is Denormalization? When is it used?", answer: "Denormalization adds redundancy to a normalized database to improve read performance. Used in data warehouses and read-heavy applications where query speed is critical.", difficulty: "Hard" },
  ],
  "Operating System": [
    { question: "What is a Process?", answer: "A process is a program in execution with its own memory space, PCB, and resources. It has states: New, Ready, Running, Waiting, Terminated.", difficulty: "Easy" },
    { question: "What is Deadlock?", answer: "Deadlock occurs when processes wait indefinitely for resources held by each other. Conditions: Mutual Exclusion, Hold & Wait, No Preemption, Circular Wait (all 4 must hold).", difficulty: "Medium" },
    { question: "What is the difference between Process and Thread?", answer: "A Process is an independent program with its own memory. A Thread is a lightweight unit within a process sharing the same memory space. Threads have less overhead than processes.", difficulty: "Easy" },
    { question: "What is Virtual Memory?", answer: "Virtual Memory allows processes to use more memory than physically available by using disk space as extended RAM. It uses paging/segmentation and enables memory isolation between processes.", difficulty: "Medium" },
    { question: "Explain CPU Scheduling algorithms.", answer: "FCFS (simple, convoy effect), SJF (optimal average wait, starvation), Round Robin (time quantum, fair), Priority Scheduling (starvation risk), MLFQ (multilevel feedback queue).", difficulty: "Medium" },
    { question: "What is a Semaphore? How does it differ from a Mutex?", answer: "Semaphore is a signaling mechanism with a counter, can be used by multiple threads. Mutex is a locking mechanism owned by one thread at a time. Mutex = binary semaphore with ownership.", difficulty: "Hard" },
    { question: "What is Paging in OS?", answer: "Paging divides physical memory into fixed-size frames and logical memory into pages of the same size. Eliminates external fragmentation but may cause internal fragmentation.", difficulty: "Medium" },
    { question: "What is the difference between Preemptive and Non-Preemptive scheduling?", answer: "Preemptive scheduling allows the OS to interrupt a running process (e.g., Round Robin). Non-preemptive scheduling lets a process run until it voluntarily releases the CPU (e.g., FCFS).", difficulty: "Medium" },
    { question: "What is a Context Switch?", answer: "A Context Switch saves the state (PCB) of a running process and loads the state of the next process. It has overhead — too frequent switches reduce CPU efficiency.", difficulty: "Easy" },
    { question: "What is Thrashing in OS?", answer: "Thrashing occurs when a process spends more time paging than executing because it lacks enough frames. The OS is busy swapping pages in and out, causing severe performance degradation.", difficulty: "Hard" },
  ],
  "Computer Networks": [
    { question: "What is the OSI Model?", answer: "The OSI Model is a 7-layer framework: Physical, Data Link, Network, Transport, Session, Presentation, Application. Each layer has specific responsibilities for network communication.", difficulty: "Medium" },
    { question: "What is TCP/IP?", answer: "TCP/IP is the core internet protocol suite. TCP provides reliable, ordered delivery with error checking. IP handles addressing and routing. Together they form the backbone of internet communication.", difficulty: "Medium" },
    { question: "What is the difference between TCP and UDP?", answer: "TCP is connection-oriented, reliable, ordered, slower (HTTP, FTP). UDP is connectionless, unreliable, faster, no ordering (DNS, video streaming, gaming). TCP has handshake; UDP doesn't.", difficulty: "Easy" },
    { question: "What is DNS?", answer: "DNS (Domain Name System) translates human-readable domain names (google.com) to IP addresses. It uses a hierarchical distributed database with Root, TLD, and Authoritative name servers.", difficulty: "Easy" },
    { question: "What is the difference between HTTP and HTTPS?", answer: "HTTP is unencrypted (port 80). HTTPS encrypts data using TLS/SSL (port 443). HTTPS provides authentication, data integrity, and confidentiality. Essential for sensitive data.", difficulty: "Easy" },
    { question: "What is a subnet mask? What is CIDR?", answer: "A subnet mask divides an IP address into network and host portions. CIDR (Classless Inter-Domain Routing) notation like /24 specifies how many bits are the network portion.", difficulty: "Medium" },
    { question: "What happens when you type a URL in the browser?", answer: "DNS lookup → TCP connection (3-way handshake) → TLS handshake (HTTPS) → HTTP request → server processes → HTTP response → browser renders HTML/CSS/JS → page displayed.", difficulty: "Hard" },
    { question: "What is a firewall?", answer: "A firewall monitors and filters incoming/outgoing network traffic based on security rules. Types: packet filtering, stateful inspection, application layer. Prevents unauthorized access.", difficulty: "Easy" },
    { question: "What is the difference between IPv4 and IPv6?", answer: "IPv4 uses 32-bit addresses (4.3 billion), IPv6 uses 128-bit addresses (virtually unlimited). IPv6 also improves routing efficiency, security (IPSec built-in), and eliminates NAT need.", difficulty: "Medium" },
    { question: "What is a 3-way handshake in TCP?", answer: "TCP connection establishment: SYN (client → server), SYN-ACK (server → client), ACK (client → server). Ensures both sides are ready to communicate before data transfer begins.", difficulty: "Medium" },
  ],
  "HR Interview": [
    { question: "Tell me about yourself.", answer: "Structure: Present (current role/skills), Past (relevant experience), Future (why this role). Keep it professional, 2-3 minutes, and tailored to the job.", difficulty: "Easy" },
    { question: "Why should we hire you?", answer: "Highlight 3 things: your relevant skills matching the role, a specific achievement showing impact, and enthusiasm for the company. Be confident and specific.", difficulty: "Easy" },
    { question: "What are your greatest strengths?", answer: "Pick 2-3 strengths relevant to the role, back each with a specific example. Common strong answers: problem-solving, adaptability, communication, technical skills.", difficulty: "Easy" },
    { question: "What is your biggest weakness?", answer: "Choose a real but non-critical weakness, explain steps you're taking to improve it. Shows self-awareness. Avoid clichés like 'I work too hard'.", difficulty: "Easy" },
    { question: "Where do you see yourself in 5 years?", answer: "Show ambition aligned with the company's growth. Mention developing expertise in your field, taking on more responsibility, and contributing to team/company goals.", difficulty: "Easy" },
    { question: "Tell me about a challenge you overcame.", answer: "Use STAR method: Situation (context), Task (your responsibility), Action (what you did), Result (outcome). Focus on your specific actions and quantify the result if possible.", difficulty: "Medium" },
    { question: "Why do you want to work here?", answer: "Research the company first. Mention specific things: their mission, products, culture, growth opportunities. Connect to your career goals. Shows genuine interest.", difficulty: "Easy" },
    { question: "Describe a time you worked in a team.", answer: "Use STAR method. Highlight collaboration, communication, and your specific contribution. Mention how you handled disagreements constructively and the team's final success.", difficulty: "Medium" },
    { question: "How do you handle pressure and tight deadlines?", answer: "Give a specific example. Mention: prioritization, breaking tasks down, clear communication with stakeholders about timelines, staying calm, and delivering results.", difficulty: "Medium" },
    { question: "Do you have any questions for us?", answer: "Always ask questions. Good ones: What does success look like in this role? What are the biggest challenges? How does the team collaborate? What is the growth path? Shows engagement.", difficulty: "Easy" },
  ],
};
